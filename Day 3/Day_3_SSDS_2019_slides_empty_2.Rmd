...
data_in %>% ggplot(aes(x = )) + geom_density(fill = 'dodgerblue') + geom_vline(xintercept = (data_in$Happiness_score)) + theme_bw()
...
```{r, eval = F, tidy = T, warning = F}
corrplot(cor_mx$r, method = "", number.cex = 0.5)
...
corrplot(cor_mx$r, order  = "", p.mat = , sig.level = 0.01)
...
```{r, eval = F, tidy = T, warning = F}
pca_res <- (data_in_F_r_sc, center = F, scale. = F)
...
as.data.frame(eig_prop_cum) %>% ggplot(aes(x = as.factor(index(eig_prop_cum)), y = eig_prop_cum)) + geom_col(fill = 'steelblue')+theme_bw() + geom_hline(aes(yintercept = ))
...
ind_PCA_all %>% ggplot(aes(x = , y = , color = , label = Country_save)) + geom_point() + geom_text(vjust = 0, nudge_y = 0.5) + theme_bw()

```

## PCA analysis - get those correlations
```{r, eval = F, tidy = T, warning = F}
corref <- cor(ind_PCA_all[,13:ncol(ind_PCA_all)], ind_PCA_all[,1:12], method = '')
...
# Bartlett
bartlett_res <- (corr_mx, n = 200, diag=TRUE)
bartlett_res
...
mvn_test <- (data_in_F_r_sc_df, mvnTest = c("mardia"), multivariatePlot = "qqplot")
...
(corr_mx, n.obs = dim(data_in_F_r_sc_df)[1], fm="pa")
...
```{r, eval = F, tidy = T, warning = F}
fit_f1 <- fa(corr_mx, nfactors = , rotate = "varimax", fm="pa")
print(fit_f1)
...
```{r, eval = F, tidy = T, warning = F}
fit_f3 <- fa(corr_mx, nfactors = , rotate = "varimax", fm="pa")
print(fit_f3)
```
...
```{r, eval = F, tidy = T, warning = F}
fit_f3b <- fa(corr_mx, nfactors = 3, rotate = "", fm="pa")
print(fit_f3b)
```

## FA with 3 factors - check other rotations (quartimax)
```{r, eval = F, tidy = T, warning = F}
fit_f3c <- fa(corr_mx, nfactors = 3, rotate = "", fm="pa")
print(fit_f3c)
```
...
* Tabachnick and Fiddell (2007, p. 646) - "Look at  the  factor  correlation  matrix  for  correlations  around  .32  and  above.  If  correlations  exceed  .32,  then there is 10% (or more) overlap in variance among factors, enough variance to warrant oblique rotation unless there are compelling reasons for orthogonal rotation."
...
```{r, eval = F, tidy = T, warning = F}
# check with vss 
vss <- vss(corr_mx,title="Very Simple Structure", rotate = "oblimin", n.obs=dim(data_in_F_r_sc_df)[1], fm = "pa", SMC=FALSE)
vss  
```
...
lm_mod_FA <- lm( ~ , data = scores_val_lm);
```
...
```{r, eval = F, tidy = T, warning = F}
scores_val_lm %>% ggplot(aes(x = , y = , color = Happiness_score, label = Country_save)) + geom_point() + geom_text(vjust = 0, nudge_y = 0.5) + theme_bw()
